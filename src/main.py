...
app.config['SQLALCHEMY_DATABASE_URI'] = f"postgresql://{os.getenv('DB_USERNAME', 'postgres')}:{os.getenv('DB_PASSWORD', 'postgres')}@{os.getenv('DB_HOST', 'localhost')}:{os.getenv('DB_PORT', '5432')}/{os.getenv('DB_NAME', 'oir_dashboard')}"
...
# from src.routes.users import users_bp # Assuming users_bp might be in user.py or needs creation
...
# app.register_blueprint(users_bp, url_prefix='/api/users') # Commented out as users_bp source is unclear
